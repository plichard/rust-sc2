...
impl FromProto<ResponseGameInfo> for GameInfo {
	fn from_proto(game_info: ResponseGameInfo) -> Self {
		let start_raw = &game_info.start_raw;
		let map_size = &start_raw.map_size;
		let area = &start_raw.playable_area;
		let area_p0 = &area.p0;
		let area_p1 = &area.p1;
		...
		Self {
					...
					(
						...
						PlayerInfo {
							...
							difficulty: i
								.difficulty
								.map(|v| v.enum_value_or_default())
								.map(Difficulty::from_proto),
							ai_build: i
								.ai_build
								.map(|v| v.enum_value_or_default())
								.map(AIBuild::from_proto),
			...
			pathing_grid: PixelMap::from_proto(&start_raw.pathing_grid),
			terrain_height: Rs::new(ByteMap::from_proto(&start_raw.terrain_height)),
			placement_grid: PixelMap::from_proto(&start_raw.placement_grid),
